...
import ProductOverviewWithGridBreadcrum from "./ProductOverviewWithGridBreadcrum.vue";

const productGetStatus = ref<"pending" | "resolved" | "rejected">("pending");
...
onMounted(async () => {
  try {
    productGetStatus.value = "pending";

    const res = await fetch(import.meta.env.BASE_URL + "api/product.json");
    product.value = await res.json();

    productGetStatus.value = "resolved";
  } catch (error) {
    console.error(error);

    productGetStatus.value = "rejected";
  }
});
</script>
...
<template>
  <div class="bg-white">
    <div class="pt-6">
      <div
        v-if="productGetStatus == 'pending'"
        class="h-full flex justify-center items-center"
      >
        <p class="mx-auto">...waiting</p>
      </div>
      <div
        v-else-if="productGetStatus == 'rejected'"
        class="h-full flex justify-center items-center"
      >
        <p class="mx-auto text-red-500">Something went wrong...</p>
      </div>
      <template v-else-if="productGetStatus == 'resolved'">
        <ProductOverviewWithGridBreadcrum :breadcrumbs="product.breadcrumbs" />
        <!-- Image gallery -->
        <div
